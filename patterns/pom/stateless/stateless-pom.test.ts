...
test('UI로 아이템을 추가하고 삭제한다', async ({ page }) => {
  const name = `pom-demo-${Date.now()}`;
  await page.goto('/');

  await itemsPage.addItem(page, name);

  // 로케이터가 export되어 있어 스펙에서 auto-retry 단언을 직접 조합
  await expect(itemsPage.locators.itemRows(page).filter({ hasText: name })).toHaveCount(1);
  ...
  await expect(itemsPage.locators.itemRows(page).filter({ hasText: name })).toHaveCount(0);
});

test('없는 아이템 삭제는 조용히 넘어가지 않고 크게 실패한다', async ({ page }) => {
  await page.goto('/');
  await itemsPage.addItem(page, `pom-guard-${Date.now()}`);
